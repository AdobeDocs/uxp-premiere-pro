...
export declare type ClipProjectItem = {
  ...
  ): Promise<boolean> //Attach proxy or high resolution footage to projectItem and return if this non-undoable action is successful.
  findItemsMatchingMediaPath(
    matchString: string,
    ignoreSubclips?: boolean
  ): Promise<[]> //Returns array of projects items with media paths containing match string
  ...
  ): Promise<boolean> //Change media file path of projectItem and return if non-undoable action is successful
