...
export declare type AudioTrack = {
  subscribeToEvent(eventKey: string): boolean	//Propagates the given event on this object.
...
export declare type ClipProjectItem = {
  ...
  setInOutPoints(inPoint: TickTime, outPoint: TickTime): boolean	//Set the in or out point of the Project item
  clearInOutPoints(): boolean	//Clear the in or out point of the Project item
...
export declare type EncoderManager = {
  exportSequence(sequence: object, exportType: string, outputFile?: string, presetFile?: string, exportFull?: boolean): boolean	//Export a sequence. If no output file and preset is specified, the sequence will be exported with the applied export settings or standard export rules will be applied.
  subscribeToEvent(eventKey: string): boolean	//Propagates the given event on this object.
  isAMEInstalled: boolean	//Check if AME is installed.
}

export declare type ExecutorStatic = {
  scopeBackendAccess(callback0: () => () => void): boolean	//Execute a transaction against the Backend. The compound action to be executed against the backend should be populated by the callback parameter.
}

export declare type Executor = {
  executeUndoableTransaction(callback0: ( compoundAction: CompoundAction ) => () => void, transactionIdentifier: string, undoText: string): boolean	//Execute a transaction against the Backend. The compound action to be executed against the backend should be populated by the callback parameter.
}

export declare type ExporterStatic = {
  exportSequenceFrame(sequence: object, time: object, filename: string, filepath: string, width: number, height: number): boolean	//Exports from a sequence. Supported formats are bmp, dpx, gif, jpg, exr, png, tga and tif
}
